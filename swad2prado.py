...
Uso
----
$ python swad2prado.py examples/swad.xml examples/output/prado_out.xml \
...
Mapping
--------
Véase mapping.json. Adáptalo si cambian los nombres de nodos/atributos.
...
def build_moodle_xml_with_markers(questions: List[Question], category: str, shuffle: bool):
    ...
    for idx, q in enumerate(questions, start=1):
        ...
        qtext = ET.SubElement(qel, "questiontext", {"format": "html"})
        marker = f"[[QTEXT_{idx}]]"
        ...
        for tag in ("correctfeedback","partiallycorrectfeedback","incorrectfeedback"):
            fb = ET.SubElement(qel, tag, {"format":"html"}); ET.SubElement(fb,"text").text = ""
...
def main(argv=None) -> int:
    ...
    p.add_argument("--mapping", required=True, help="JSON con el mapeo de XPaths (ver mapping.json)")
